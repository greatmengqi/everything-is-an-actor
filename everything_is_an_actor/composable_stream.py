...
import time
from typing import (
    Any, AsyncIterator, Awaitable, Callable, Generic, Iterable, Optional,
    TypeVar,
)
...
class StreamSender(Generic[T]):
    ...
    async def put(self, element: T) -> None:
        ...
        if current is self._loop:
            ...
            fut = asyncio.run_coroutine_threadsafe(
                self._queue.put(element), self._loop,
            )
    ...
    async def complete(self) -> None:
        ...
        if current is self._loop:
            ...
            fut = asyncio.run_coroutine_threadsafe(
                self._queue.put(_STREAM_DONE), self._loop,
            )
            await asyncio.wrap_future(fut)
    ...
    async def fail(self, error: Exception) -> None:
        ...
        if current is self._loop:
            ...
            fut = asyncio.run_coroutine_threadsafe(
                self._queue.put(sentinel), self._loop,
            )
            await asyncio.wrap_future(fut)
...
class ComposableStream(Generic[T]):
    ...
    def map_async(
        self, fn: Callable[[T], Awaitable[U]], parallelism: int = 1,
    ) -> ComposableStream[U]:
    ...
    def recover_with(
        self, fn: Callable[[Exception], ComposableStream[T]],
    ) -> ComposableStream[T]:
            ...
            async for item in source:
                if time.monotonic() > deadline:
                    raise asyncio.TimeoutError(
                        f"Stream did not complete within {seconds}s"
                    )
                yield item
    ...
    def flat_map_merge(
        self, fn: Callable[[T], ComposableStream[U]], breadth: int = 16,
    ) -> ComposableStream[U]:
    ...
    def watch_termination(
        self, callback: Callable[[Optional[Exception]], Any],
    ) -> ComposableStream[T]:
    ...
    @staticmethod
    def unfold_async(
        seed: U, fn: Callable[[U], Awaitable[Optional[tuple[T, U]]]],
    ) -> ComposableStream[T]:
            ...
            while True:
                try:
                    item = await ComposableFuture(
                        source.__anext__(), loop=source_loop,
                    )
                    yield item
                except StopAsyncIteration:
                    break
